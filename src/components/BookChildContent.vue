  ...
  <div>
    <div class="content">
      <b-field>
        <b-button
          style="height: auto"
          :class="isViewed ? 'is-success' : 'is-primary'"
          class="is-medium book-child"
          outlined
          expanded
          @click="isOpenBook = true"
        >
          {{ wikiBook }}
        </b-button>
        <p class="control">
    ...
    <b-modal v-model="isOpenBook" :width="1000" scroll="keep">
      <header class="modal-card-head">
        ...
        <b-checkbox v-model="isUrlAllow"> 保存超連結 </b-checkbox>
      </header>
      ...
      <footer class="modal-card-foot">
        <section>
          <b-field>
            <b-button type="is-success" outlined @click="sendWikiCutObj"
              >確認分件方式</b-button
            >
          </b-field>
        </section>
...
<script>
import { parseHtmlText, getWikiPage, splitAriaConvert } from "../tool.js";
...
export default {
  ...
  created: async function () {
    ...
    this.viewContents.push(
      this.wikiText.hyperlinks.replace(
        /\n{0,1}<Udef_wiki[^<]*>\n*([^<]*)\n*<\/Udef_wiki>\n{0,1}/gm,
        "$1"
      )
    );
