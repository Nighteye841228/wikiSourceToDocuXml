  ...
  <div ref="tagParent" class="column is-one-third">
    ...
    <b-modal v-model="isOpenTagModal" :width="1000" scroll="keep">
      <header class="modal-card-head">
        ...
        <nav class="level">
          <div class="level-item" style="padding-right: 0.5em">
            <h6>選擇Tag名稱，劃記後確認。再次點擊Tag即可刪除</h6>
          </div>
          <div class="level-item">
            <b-field>
              <b-select
      ...
      <footer class="modal-card-foot">
        ...
        <b-button type="is-primary" outlined @click="isOpenTagModal = false"
          >取消</b-button
        >
      </footer>
    </b-modal>
...
export default {
  ...
  methods: {
    ...
    print: function (evt) {
      ...
      const { width } = this.wordSelection
        .getRangeAt(0)
    ...
    deleteTag: function (evt) {
      this.$refs.text.insertBefore(
        document.createTextNode(evt.target.innerText),
        evt.target
      );
    ...
    setTag: function () {
      ...
      this.extractCompleteTags[this.selectTag] = this.extractCompleteTags[
        this.selectTag
      ]
    ...
    searchAndTag: function (tagName, tagWord) {
      this.$refs.text.innerHTML = searchAndTag(
        tagName,
        tagWord,
