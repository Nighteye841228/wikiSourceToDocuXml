...
var ClsDocuskyManageDbListSimpleUI = function(param) {       // constructor
    ...
    me.widgetEvents = {
        dbClick: null,     // null: disable clicking db -- can be set from invoker as obj.widgetEvents.dbClick=function(s){...}, where s will be the db title
    ...
    var displayDbList = function(evt, succFunc) {
        ...
        $('#' + uploadXmlFileId).change(function(e) {
            ...
            var uploadXmlToBuildDbId = me.idPrefix + 'uploadXmlToBuildDb' + me.uniqueId;
            $('#' + loadingContainerId).css({
                top: $('#' + dbListContentId).position().top + $('#' + dbListContentId).height()/2,
                left: $('#' + dbListContentId).position().left + $('#' + dbListContentId).width()/2
            });
            $('#' + loadingContainerId).show();
    ...
    me.getUserProfile = function(evt, succFunc, failFunc) {
        ...
        var url = me.urlGetUserProfileJson + (SessionKey ? ('?' + SessionKey) : '');        // 2021-02-06
...
export { docuskyManageDbListSimpleUI };